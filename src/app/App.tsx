  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between items-center">
        <span className="text-xs font-medium" style={{ color: C.fg2 }}>{label}</span>
        <span className="text-xs font-semibold" style={{ color: C.fg }}>
          {current}<span style={{ color: C.fg3 }}>/{goal}{unit}</span>
        </span>
      </div>
...
function Badge({ children, color = C.accentDim, textColor = C.accent }: {
...
function SectionHeader({ title, action, onAction }: {
  ...
  return (
    <div className="flex justify-between items-center mb-3">
      <h3 className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: C.fg2 }}>{title}</h3>
      {action && (
        <button className="text-xs font-semibold" style={{ color: C.accent }} onClick={onAction}>
          {action}
        </button>
      )}
    </div>
...
function SubScreenHeader({ title, onBack }: { title: string; onBack: () => void }) {
  return (
    <div className="flex items-center gap-3 px-5 pt-5 pb-4">
      ...
      <h2 className="text-xl font-bold" style={{ color: C.fg }}>{title}</h2>
    </div>
  );
}
...
function DashboardScreen({ onNavigate }: { onNavigate: (s: Screen) => void }) {
  ...
  const macros = { cal: 1840, calGoal: 3100, protein: 142, proteinGoal: 220, carbs: 180, carbsGoal: 310, fat: 48, fatGoal: 85 };
  const water = 1.8;
  const waterGoal = 3.5;
  ...
  return (
    <div className="px-5 pb-8 pt-3">
      {/* Greeting */}
      <div className="mb-6">
        <p className="text-sm font-semibold mb-1" style={{ color: C.accent }}>Good morning</p>
        <h1 className="text-[30px] font-extrabold leading-none tracking-tight" style={{ color: C.fg }}>
          Alex Morrison
        </h1>
        <p className="text-sm mt-1.5" style={{ color: C.fg3 }}>{dateStr}</p>
      </div>
      ...
      <div className="flex gap-3 mb-5">
        <div className="flex-1 rounded-[20px] p-4" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          <div className="flex items-center gap-1 mt-2">
            ...
            <span className="text-[11px] font-semibold" style={{ color: C.accent }}>+0.3 this week</span>
          </div>
        </div>
        <div className="flex-1 rounded-[20px] p-4" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          <p className="text-[11px] mb-1.5" style={{ color: C.fg2 }}>Goal</p>
          <p className="text-[26px] font-bold leading-none" style={{ color: C.fg }}>
            86.0<span className="text-sm font-medium ml-1" style={{ color: C.fg3 }}>kg</span>
          </p>
          <div className="flex items-center gap-1 mt-2">
            <Target size={12} color={C.fg3} />
            <span className="text-[11px]" style={{ color: C.fg3 }}>3.6 kg to go</span>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[22px] p-4 mb-5" style={{ background: C.card, border: `1px solid ${C.border}` }}>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm font-semibold mt-1" style={{ color: C.fg }}>
              Avg 2,863 kcal
              <span className="font-normal ml-2" style={{ color: C.fg3 }}>/ 3,100 target</span>
            </p>
          </div>
        ...
        <div style={{ height: 80 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={weeklyBarData} barSize={20} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
              <XAxis
              ...
              <Bar dataKey="cal" radius={[5, 5, 0, 0]}>
                {weeklyBarData.map((entry, i) => (
                  <Cell key={i} fill={entry.cal >= TARGET_CAL ? C.accent : C.accentDim} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        {[
          { label: "Calories", remaining: macros.calGoal - macros.cal, unit: "kcal", color: C.amber, pct: macros.cal / macros.calGoal },
          { label: "Protein", remaining: macros.proteinGoal - macros.protein, unit: "g", color: C.accent, pct: macros.protein / macros.proteinGoal },
          { label: "Carbs", remaining: macros.carbsGoal - macros.carbs, unit: "g", color: C.blue, pct: macros.carbs / macros.carbsGoal },
          { label: "Fat", remaining: macros.fatGoal - macros.fat, unit: "g", color: C.purple, pct: macros.fat / macros.fatGoal },
          ...
          <div key={label} className="rounded-[18px] p-4" style={{ background: C.card, border: `1px solid ${C.border}` }}>
            ...
            <p className="text-[10px] mt-1.5" style={{ color: C.fg3 }}>{unit} left</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-[22px] p-4 mb-5 flex items-center gap-5" style={{ background: C.card, border: `1px solid ${C.border}` }}>
        <ProgressRing
          value={macros.cal} max={macros.calGoal} size={76} stroke={6} color={C.accent}
        >
        ...
        <div className="flex-1">
          <p className="text-xs font-semibold mb-1" style={{ color: C.fg }}>Daily Goal Progress</p>
          <p className="text-[11px] mb-2.5" style={{ color: C.fg3 }}>
            {macros.cal.toLocaleString()} of {macros.calGoal.toLocaleString()} kcal consumed
          </p>
          <div className="flex flex-col gap-1.5">
            {[
              { label: "Protein", pct: macros.protein / macros.proteinGoal, color: C.accent },
              { label: "Carbs", pct: macros.carbs / macros.carbsGoal, color: C.blue },
              { label: "Fat", pct: macros.fat / macros.fatGoal, color: C.purple },
            ].map(({ label, pct, color }) => (
      ...
      <div className="rounded-[20px] mb-5 overflow-hidden" style={{ background: C.card, border: `1px solid ${C.border}` }}>
        {mealStatus.map(({ name, done }, i) => (
          ...
          >
            ...
            {done
              ? <span className="text-[11px]" style={{ color: C.fg3 }}>Logged</span>
              : <span className="text-[11px]" style={{ color: C.fg3 }}>Pending</span>
            }
          </div>
        ))}
      </div>
        ...
        <div className="flex items-center justify-between mb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold" style={{ color: C.fg }}>
              {water}L
              <span className="font-normal text-sm ml-1" style={{ color: C.fg3 }}>of {waterGoal}L</span>
            </span>
          </div>
