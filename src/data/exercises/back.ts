import type { ExerciseDefinition } from "@/types/workout";

export const backExercises: ExerciseDefinition[] = [
  {
    id: "pull-up",
    name: "Pull-up",
    aliases: ["Strict Pull-up", "Bodyweight Pull-up", "Zhyby", "Zhyb"],
    primaryMuscle: "back",
    secondaryMuscles: ["biceps", "abs"],
    equipment: "bodyweight",
    category: "compound",
    difficulty: "intermediate",
    defaultRestSeconds: 150,
    defaultSets: 4,
    defaultReps: 8,
    media: {
      start: "/exercises/pull-up/start.png",
      finish: "/exercises/pull-up/finish.png",
      thumbnail: "/exercises/pull-up/start.png",
    },
    progression: {
      minReps: 6,
      maxReps: 10,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      biceps: 55,
      abs: 25,
    },
    instructions: [
      "Grip the bar slightly wider than shoulder width.",
      "Start from a dead hang with shoulders controlled.",
      "Pull your chest toward the bar.",
      "Drive elbows down and back.",
      "Lower under control to full arm extension.",
    ],
    tips: [
      "Avoid swinging.",
      "Lead with the chest.",
      "Control the lowering phase.",
    ],
    mistakes: [
      "Using momentum.",
      "Half reps.",
      "Shrugging at the top.",
    ],
    tags: ["vertical pull", "bodyweight", "back"],
  },
  {
    id: "lat-pulldown",
    name: "Lat Pulldown",
    aliases: ["Wide Grip Pulldown"],
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "machine",
    category: "compound",
    difficulty: "beginner",
    defaultRestSeconds: 120,
    defaultSets: 3,
    defaultReps: 10,
    media: {
      start: "/exercises/lat-pulldown/start.png",
      finish: "/exercises/lat-pulldown/finish.png",
      thumbnail: "/exercises/lat-pulldown/start.png",
    },
    progression: {
      minReps: 8,
      maxReps: 12,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      biceps: 45,
    },
    instructions: [
      "Sit with thighs secured under the pad.",
      "Grip the bar slightly wider than shoulder width.",
      "Pull the bar toward your upper chest.",
      "Keep chest lifted and elbows driving down.",
      "Return slowly until arms are extended.",
    ],
    tips: [
      "Think elbows down, not hands down.",
      "Do not lean back excessively.",
      "Keep tension at the top.",
    ],
    mistakes: [
      "Pulling behind the neck.",
      "Using too much momentum.",
      "Turning it into a biceps exercise.",
    ],
    tags: ["vertical pull", "machine", "lats"],
  },
  {
    id: "barbell-row",
    name: "Barbell Row",
    aliases: ["Bent-over Barbell Row"],
    primaryMuscle: "back",
    secondaryMuscles: ["biceps", "glutes"],
    equipment: "barbell",
    category: "compound",
    difficulty: "intermediate",
    defaultRestSeconds: 150,
    defaultSets: 4,
    defaultReps: 8,
    media: {
      start: "/exercises/barbell-row/start.png",
      finish: "/exercises/barbell-row/finish.png",
      thumbnail: "/exercises/barbell-row/start.png",
    },
    progression: {
      minReps: 6,
      maxReps: 10,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      biceps: 45,
      glutes: 25,
    },
    instructions: [
      "Hinge at the hips with a neutral spine.",
      "Hold the bar just outside shoulder width.",
      "Pull the bar toward your lower ribs.",
      "Squeeze shoulder blades together.",
      "Lower the bar under control.",
    ],
    tips: [
      "Keep torso angle stable.",
      "Brace your core hard.",
      "Pull elbows back, not up.",
    ],
    mistakes: [
      "Jerking the weight.",
      "Rounding the lower back.",
      "Standing too upright.",
    ],
    tags: ["horizontal pull", "barbell", "strength"],
  },
  {
    id: "db-row",
    name: "DB Row",
    aliases: ["One-arm Dumbbell Row"],
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "dumbbell",
    category: "compound",
    difficulty: "beginner",
    defaultRestSeconds: 120,
    defaultSets: 3,
    defaultReps: 10,
    media: {
      start: "/exercises/db-row/start.png",
      finish: "/exercises/db-row/finish.png",
      thumbnail: "/exercises/db-row/start.png",
    },
    progression: {
      minReps: 8,
      maxReps: 12,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      biceps: 40,
    },
    instructions: [
      "Support one hand and knee on a bench.",
      "Let the dumbbell hang below your shoulder.",
      "Pull the dumbbell toward your hip.",
      "Pause briefly at the top.",
      "Lower under control.",
    ],
    tips: [
      "Keep shoulders square.",
      "Avoid twisting your torso.",
      "Pull toward the hip for more lat focus.",
    ],
    mistakes: [
      "Rotating too much.",
      "Shrugging the shoulder.",
      "Using momentum.",
    ],
    tags: ["horizontal pull", "dumbbell", "unilateral"],
  },
  {
    id: "seated-cable-row",
    name: "Seated Cable Row",
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "cable",
    category: "compound",
    difficulty: "beginner",
    defaultRestSeconds: 120,
    defaultSets: 3,
    defaultReps: 10,
    media: {
      start: "/exercises/seated-cable-row/start.png",
      finish: "/exercises/seated-cable-row/finish.png",
      thumbnail: "/exercises/seated-cable-row/start.png",
    },
    progression: {
      minReps: 8,
      maxReps: 12,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      biceps: 40,
    },
    instructions: [
      "Sit tall with feet braced.",
      "Start with arms extended and chest lifted.",
      "Pull the handle toward your lower ribs.",
      "Squeeze your shoulder blades together.",
      "Return slowly without rounding forward.",
    ],
    tips: [
      "Keep the torso stable.",
      "Use a full stretch and squeeze.",
      "Do not yank the cable.",
    ],
    mistakes: [
      "Leaning back too much.",
      "Rounding forward excessively.",
      "Pulling with only the arms.",
    ],
    tags: ["horizontal pull", "cable", "back"],
  },
  {
    id: "chest-supported-row",
    name: "Chest Supported Row",
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "machine",
    category: "compound",
    difficulty: "beginner",
    defaultRestSeconds: 120,
    defaultSets: 3,
    defaultReps: 10,
    media: {
      start: "/exercises/chest-supported-row/start.png",
      finish: "/exercises/chest-supported-row/finish.png",
      thumbnail: "/exercises/chest-supported-row/start.png",
    },
    progression: {
      minReps: 8,
      maxReps: 12,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      biceps: 35,
    },
    instructions: [
      "Set the chest pad so your torso is stable.",
      "Grip the handles with shoulders relaxed.",
      "Pull handles toward your torso.",
      "Squeeze the upper back.",
      "Lower slowly to a full stretch.",
    ],
    tips: [
      "Great option to reduce lower-back fatigue.",
      "Keep chest against the pad.",
      "Pause briefly at peak contraction.",
    ],
    mistakes: [
      "Lifting chest off the pad.",
      "Using momentum.",
      "Shortening the range of motion.",
    ],
    tags: ["machine", "horizontal pull", "upper back"],
  },
  {
    id: "straight-arm-pulldown",
    name: "Straight Arm Pulldown",
    aliases: ["Cable Pullover"],
    primaryMuscle: "back",
    secondaryMuscles: ["triceps"],
    equipment: "cable",
    category: "isolation",
    difficulty: "intermediate",
    defaultRestSeconds: 75,
    defaultSets: 3,
    defaultReps: 12,
    media: {
      start: "/exercises/straight-arm-pulldown/start.png",
      finish: "/exercises/straight-arm-pulldown/finish.png",
      thumbnail: "/exercises/straight-arm-pulldown/start.png",
    },
    progression: {
      minReps: 12,
      maxReps: 15,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      triceps: 20,
    },
    instructions: [
      "Set the cable high with a straight bar or rope.",
      "Stand slightly back with arms nearly straight.",
      "Pull the attachment down toward your thighs.",
      "Squeeze your lats at the bottom.",
      "Return slowly to the stretched position.",
    ],
    tips: [
      "Keep elbows softly bent but fixed.",
      "Think about pulling with your lats.",
      "Do not turn it into a triceps pressdown.",
    ],
    mistakes: [
      "Bending elbows too much.",
      "Using body momentum.",
      "Losing the stretch at the top.",
    ],
    tags: ["lat isolation", "cable", "pullover"],
  },
  {
    id: "deadlift",
    name: "Deadlift",
    aliases: ["Conventional Deadlift", "Mŕtvy ťah"],
    primaryMuscle: "back",
    secondaryMuscles: ["legs", "glutes", "fullBody"],
    equipment: "barbell",
    category: "compound",
    difficulty: "advanced",
    defaultRestSeconds: 240,
    defaultSets: 3,
    defaultReps: 5,
    media: {
      start: "/exercises/deadlift/start.png",
      finish: "/exercises/deadlift/finish.png",
      thumbnail: "/exercises/deadlift/start.png",
    },
    progression: {
      minReps: 3,
      maxReps: 6,
      weightStep: 5,
    },
    activation: {
      back: 100,
      glutes: 80,
      legs: 70,
      fullBody: 100,
    },
    instructions: [
      "Stand with the bar over mid-foot.",
      "Grip the bar just outside your legs.",
      "Set your back neutral and brace hard.",
      "Push the floor away and stand tall.",
      "Lower the bar under control with hips back.",
    ],
    tips: [
      "Keep the bar close to your body.",
      "Brace before every rep.",
      "Push through the floor, do not yank.",
    ],
    mistakes: [
      "Rounding the lower back.",
      "Starting with hips too low.",
      "Letting the bar drift forward.",
    ],
    tags: ["hinge", "barbell", "strength", "full body"],
  },
  {
    id: "t-bar-row",
    name: "T-Bar Row",
    aliases: ["Chest-Supported T-Bar Row", "Landmine Row"],
    primaryMuscle: "back",
    secondaryMuscles: ["biceps"],
    equipment: "machine",
    category: "compound",
    difficulty: "intermediate",
    defaultRestSeconds: 120,
    defaultSets: 4,
    defaultReps: 10,
    media: {
      start: "/exercises/t-bar-row/start.png",
      finish: "/exercises/t-bar-row/finish.png",
      thumbnail: "/exercises/t-bar-row/start.png",
    },
    progression: {
      minReps: 8,
      maxReps: 12,
      weightStep: 5,
    },
    activation: {
      back: 100,
      biceps: 35,
    },
    instructions: [
      "Straddle the machine and hinge forward at the hips.",
      "Grip the handles with a neutral or close grip.",
      "Pull the weight toward your lower chest.",
      "Squeeze your shoulder blades together at the top.",
      "Lower under control without rounding your back.",
    ],
    tips: [
      "Keep your chest up and back flat throughout.",
      "Drive the pull with your elbows, not your hands.",
      "Pause briefly at the top of each rep.",
    ],
    mistakes: [
      "Rounding the lower back.",
      "Using momentum to heave the weight up.",
      "Not pulling all the way to the torso.",
    ],
    tags: ["row", "back", "machine", "strength"],
  },
  {
    id: "dumbbell-shrug",
    name: "Dumbbell Shrug",
    aliases: ["Shrugs", "DB Shrug"],
    primaryMuscle: "back",
    secondaryMuscles: ["forearms"],
    equipment: "dumbbell",
    category: "isolation",
    difficulty: "beginner",
    defaultRestSeconds: 90,
    defaultSets: 3,
    defaultReps: 15,
    media: {
      start: "/exercises/dumbbell-shrug/start.png",
      finish: "/exercises/dumbbell-shrug/finish.png",
      thumbnail: "/exercises/dumbbell-shrug/start.png",
    },
    progression: {
      minReps: 12,
      maxReps: 20,
      weightStep: 2.5,
    },
    activation: {
      back: 100,
      forearms: 20,
    },
    instructions: [
      "Stand tall holding a dumbbell in each hand at your sides.",
      "Keep your arms straight throughout the movement.",
      "Elevate your shoulders straight up toward your ears.",
      "Hold briefly at the top, squeezing your traps.",
      "Lower under control back to the start.",
    ],
    tips: [
      "Move straight up and down — don't roll your shoulders.",
      "Avoid using your arms or elbows to assist the lift.",
      "A slight pause at the top improves the contraction.",
    ],
    mistakes: [
      "Shrugging at the top.",
      "Using momentum or bouncing the weight.",
      "Bending the elbows to cheat the weight up.",
    ],
    tags: ["shrug", "traps", "isolation"],
  },
];